...
## 修正3: スマホ対応 固定お問い合わせバー（sticky footer）

### 概要
ページ下部にスクロールしても常に表示される固定バー（sticky footer）を追加。
電話ボタンとお問い合わせボタンを配置し、どのページ位置からでもワンタップで問い合わせ可能にする。

### 追加した要素

1. **デモバナー**（ページ最上部・fixed）
   - テキスト: 「修正イメージデモ — 株式会社CAPSULE」
   - `position: fixed; top: 0; z-index: 100000`

2. **Sticky Footer**（ページ最下部・fixed）
   - 電話ボタン: `tel:03-5797-9487`（タップで発信）
   - お問い合わせボタン: お問い合わせフォームへリンク（別タブ）
   - `position: fixed; bottom: 0; z-index: 99999`
   - 青系グラデーション背景（`#1a3a6b` 〜 `#2563a8`）
   - スマホ対応（600px以下でサイズ・余白調整）

### 技術的な修正方法
- CSS クラスは全て `capsule-` プレフィックス付き（既存クラスとの衝突回避）
- 新しい `<style>` ブロックをドキュメント先頭付近に追加
- 新しい HTML 要素をドキュメント末尾に追加
- 既存の HTML 要素・CSS は一切変更なし

### バックアップ
- `original.html`: 修正前のオリジナルファイル
